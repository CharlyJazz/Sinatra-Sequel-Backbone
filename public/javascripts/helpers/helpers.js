...
(function ($) {
  ...
  TagsMaterialize.prototype = {
    ...
    add : function (item, dontPushVal, options) {
      ...
      // add a tag element with the close icon
      var $tag = $('<span class="' + htmlEncode(tagClass) +
                  (itemTitle !== null ? ('" title="' + itemTitle) : '') +
                  '">' + htmlEncode(itemText) +
                  '<i class="fa fa-close" data-role="remove"></i></span>');
...
// https://github.com/rotundasoftware/underscore-template-helpers
...
_.addTemplateHelpers({
  user_authenticated_is_the_owner: function(id) {
    /*
    * Check if the user authenticated if the owner
    * of the any snippet, comment, proyect, etc
    * */
    if (app.current_user.is_authenticated() && app.current_user.get('id') == id) {
      return true;
    }
    return false;
  }
