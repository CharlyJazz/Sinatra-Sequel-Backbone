...
describe CommentProyect do
  before :each do
    @user_have_proyect = User.new(:name => "SnippetMan", :email=>"SnippetMan@gmail.com", :password=>"123456", :password_confirmation=>"123456").save
    @user_comment = User.new(:name => "Audrey", :email=>"Audrey@gmail.com", :password=>"123456", :password_confirmation=>"123456").save
    @proyect = Proyect.new(:name=>"github", :description=>"rails web for git repositories", :user_id=>@user_have_proyect.id).save
  end
  ...
  context "create comment" do
    before :each do
      @user_comment.add_comment_proyect(:body=>"Sad coding lorem ipsum", :proyect_id=>@proyect.id)
    end
    it 'should create' do
      expect(CommentProyect.where(:proyect_id=>@proyect.id).count).to eq(1)
    end
    context "delete comment" do
      let(:comment) {CommentProyect.first(:body=>"Sad coding lorem ipsum", :proyect_id=>@proyect.id)}
