...
describe Proyect do
  before(:each) do
    ...
    @proyect = Proyect.new(:name=>"github", :description=>"rails web for git repositories", :user_id=>@user.id).save
  end
  ...
  context "add snippet with the proyect_has_snippet model" do
    before(:each) do
      ...
      ProyectHasSnippet.create(:proyect_id=>@proyect.id, :snippet_id=>@snippet.id)
    end
  ...
  context "especial method" do
    ...
    context "remove snippet" do
      ...
      it "should remove snippet" do
        expect(ProyectHasSnippet.first(:proyect_id=>@proyect.id, :snippet_id=>@snippet.id)).to be_kind_of(ProyectHasSnippet)
        Proyect.remove_snippets(@proyect, [@snippet]) # Delete
        expect(ProyectHasSnippet.first(:proyect_id=>@proyect.id, :snippet_id=>@snippet.id)).to_not be_kind_of(ProyectHasSnippet)
      end
    end
  end
end