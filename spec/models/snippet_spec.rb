...
describe Snippet do
  ...
  describe "create snippet"
    it "should raise error" do
        expect{Snippet.new(:filename => "name", :body=>"Lorem ipsum...", :user_id=>@user.id).save}.to raise_error(Sequel::ValidationFailed)
    end
    it "should create" do
        expect{Snippet.new(:filename => "name.py", :body=>"Lorem ipsum...", :user_id=>@user.id).save}.to_not raise_error(Sequel::ValidationFailed)
    end
  describe "detected language of a file" do
    before :each do
